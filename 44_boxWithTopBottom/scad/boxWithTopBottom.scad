...
boxWithBottom(px=20);
boxWithTop(rx=180);
module boxWithBottom(px=0, py=0, pz=0, rx=0, ry=0, rz=0, sht=0.3){
    translate([(px), (py), pz])
    rotate([rx,ry,rz]){
        difference(){
            yCube(20,20,20);
        ...
        //yPoly(p=[[5,0],[4,1],[-4,1],[-5,0],[-4,-1],[4,-1]],szz=10, py=9,pz=-10);
           
    }//transform
}//module

module boxWithTop(px=0, py=0, pz=0, rx=0, ry=0, rz=0, sht=0.3){
    translate([(px), (py), pz])
    rotate([rx,ry,rz]){
        union(){
            difference(){            
